...
var _=require('underscore');//要使用过该模块中的findWhere方法通过对象的属性值找到该对象并返回
var app=express();
...
app.post('/useradd',urlencodedParser,function(req,res){
    user.select('tel','users',function(data){
        if(data.length!=0){
            ...
            // console.log(regtime)
            var arr=[req.body.username,req.body.passwd,req.body.tel,req.body.address,req.body.sex,regtime]
...
io.on('connection',function(socket){
    ...
    var n=1;
    user.select('COUNT(oid) AS num','orders',function(num){
        n=num[0].num;
...
app.get('/index',function (req,res) {
    ...
    User.init();
    //开始执行数据库操作
    User.query(0,'users',function (err,data) {
        var inforData={
            index :data
        };
...
app.post('/zhuce',urlencodedParser,function (req,res) {
    ...
    dao.getName(tel,function (data) {
        ...
        if (data.length==0){
            ...
            console.log("插入数据！！！");
            dao.insert(name,pwd,tel,function () {
